...
Iapyx::Iapyx(String host, int port, String jobName, String params, bool debug=false) {
  this->host = host;
  this->port = port;
...
bool Iapyx::startBuild() {
  ...
  //                BRANCH + String("&token=") + TOKEN + String("&cause=") + CAUSE;
  ...
  while (keepTrying) {
    ...
    if (response.status == 201) {
      keepTrying = false;
      buildStarted = true;
      if (debugMode) Serial.println("*** Build Started ***");
    } else {
      if (debugMode) {
        Serial.print("Status: ");
      ...
      if (retryNum > 5) {
        keepTrying = false;
        if (debugMode) Serial.println("Retries Maxed Out.");
      } else {
        if (debugMode) Serial.println("Retrying...");
        retryNum++;
    ...
    if (response.status == 200) {
      ...
      if (isLatestBuildInfo(root)) {
        ...
        if (retryNum >= 10) {
          ...
          if (debugMode) Serial.println("Not current build status. Sleeping. Will Retry.");
          progress(DIRECTION_CLOCKWISE, 1000, PROGRESS_COLOR);
        }
        retryNum++;
      }
...
void Iapyx::startJobAndMonitor() {
  ...
  if (startBuild()) {
    ...
    if (debugMode) Serial.println("---> Error Starting Build <---");
    OK = false;
  }
